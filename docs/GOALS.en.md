...
| M3 ✅ | Bytecode VM (with **coroutine scheduler and channel runtime**; compiling to C/LLVM as needed later, completing the borrow checker) | Performance ≥ same order of magnitude as CPython (completed 2026-08-14: AST→bytecode compiler + stack VM; `go`/`task_group`/`Chan[T]` coroutine runtime; after zero-copy calls + reference dispatch + run-until-block scheduling, full-program fib+sum_loop ~1.0x CPython, with fib/sum_loop each beating it; see bench/README) |
| M4 ✅ | Standard library core + formatter + LSP | Can complete a real small project (completed 2026-08-15: see changelog; acceptance project `examples/projects/json_tool`) |
...
| 2026-08-14 | **M3 complete**: eval migrated to a bytecode VM (new `compiler.rs` AST→bytecode, `vm.rs` stack VM); coroutine scheduler and channel runtime (`go`/`task_group`/`Chan[T]` send/recv/close, buffered/unbuffered, `for v in ch`, `yield`); performance work: locals value semantics + lazy borrow cells, `Value` shrunk to 24B (`Str` as `Rc<str>`, boxed `Struct`), `&Instr` reference dispatch + int fast paths, run-until-block cooperative scheduling (aligned with §7.3 "switching only at channel ops and yield"), zero-copy calls (locals live on the task stack, `Frame.base` indexing), current-function code caching, `CallMethod(m, argc)` encoding; new `bench/` vs CPython | Aligns with the M3 acceptance "performance ≥ same order of magnitude as CPython": fib(28)+sum_loop(1e6) full program ~1.0x CPython (initial 3.7-4.4x), fib and sum_loop each beating it; details in bench/README |
| 2026-08-14 | **M4 stage 0 (language foundation)**: decided D8-D12 and implemented — `Option[T]`/`Result[T,E]` construction and unwrapping (Some/None/Ok/Err, is_some/is_none/is_ok/is_err/unwrap, error codes E0219/E0220); generic functions with the `Comparable` bound (E0320), call-site instantiation; `Dict[K,V]` (indexing/methods, get returns Option), `Set[T]` (unique elements), `Tuple` (indexing); str method set (len/sub/split/join/contains/starts_with/ends_with/to_str/to_int/to_float); `test` blocks + `sole test` runner + `assert` statements (E0221); structural compatibility with `Unknown` holes (Option[?] ~ Option[int]) | M4 foundation: error handling/generics/collections/str/testing primitives, all with test coverage |
| 2026-08-14 | **M4 stage 1 (stdlib core)**: decided D13 and implemented — `import`/`from` module mechanism (multi-file loading, cycle dedup, prefix rewriting, from-name validation); stdlib builtins: file IO (read_to_str/write), time (clock/sleep), math (abs/floor/ceil/round/sqrt/pow), JSON (json_encode/json_decode + dynamic `Json` type); `None` as a "hole" in collection literals for heterogeneous JSON literals | Modularity + stdlib core usable end to end, unit-test covered; HTTP/Shared/Mutex deferred per D13 |
| 2026-08-14 | **M4 stage 2-3 (formatter + LSP)**: decided D14 and implemented — formatter (idempotent + semantics-preserving, verified over all examples) with `sole fmt`/`--check` (wired into CI); LSP server (initialize/shutdown, full-sync diagnostics reusing the type checker with bilingual rendering, go-to-definition, completion, 6 process-level integration tests) | Toolchain: canonical format and editor support usable; deterministic formatting and instant diagnostics AI can rely on |
| 2026-08-15 | **M4 complete (wrap-up)**: `and`/`or`/`not` boolean operators landed; extra methods — str `trim`, List `contains`, Result `unwrap_err`, Json `len`/`contains`/`keys`/`is_int`/`is_str`/`to_str`; generic call-site instantiation rewritten (substitute_typevars/collect_bindings); the lexer now supports scientific-notation float literals (`1e300`/`1.5e-3`/`2E+10`; `1e` falls back to int + identifier); **if-branch merging**: a move inside a diverging branch (one that always returns) no longer affects the state after the `if`, while moves in non-diverging branches still propagate (E0401 preserved); borrows still die with their branch; fixed a committed-code `-x` compilation bug (a stray push shifted call arguments, so `f(a, -b)` received scrambled args); example library expanded (collections/concurrency/generics/json_usage/option/str_methods); acceptance project `examples/projects/json_tool` (a JSON CLI tool: multi-file modules + stdlib + 41 `test` cases, all green). Honestly recorded: the CLI does not yet pass arguments to scripts (the demo drives `main` from `test` blocks); `x = f(x)` self-assignment is still rejected by the borrow checker when `x` appears inside call arguments (the skip mechanism does not pierce calls; workarounds are temp bindings or `mut ref` accumulation) | The M4 acceptance "complete a real small project" lands; `sole run` argument passing and self-assignment deferred to M5 |
| 2026-08-15 | **Tech debt round 1 (correctness first)**: VM runtime errors now carry the source line (new `Function.lines` line table; the compiler records each instruction's source line and the VM looks it up via `line_at` on error; the 8 method-dispatch helpers and `set_global` thread a line parameter; zero runtime cost); new dedicated `Instr::Neg` (replaces the `0 - x` hack; each negation 3→2 instructions); List/Tuple index out of range changed from silent `Unit` to runtime error E0223 (with line); `Json`-vs-`None` comparison supported (missing `cmp_op` branch added, aligning with D13); 8 mojibake comments cleared; doc-vs-implementation audit: D6 "for translates to iteration interface calls" downgraded (builtin containers only for now; recorded in the debt list); error-location wording updated (runtime errors carry the line); new `docs/DEBT.md` inventory (C/D/P/S sections, linked from GOALS) | Correctness first: errors locatable, behavior predictable; remaining debt (arbitrary-precision int, hashed containers, index borrows, select/Shared/Mutex, LSP incremental, CLI args, self-assignment, file splits) all logged for later rounds |
